...
(
    select t.request_at, cast(count(t.id)as decimal(5,2)) as total_req from trips t
    inner join users c
...
(
    select t.request_at, cast(count(t.id)as decimal(5,2)) as canl_req from trips t
    inner join users c
    on c.users_id = t.client_id and c.banned = 'No'
    inner join users d
    on d.users_id = t.driver_id and d.banned = 'No'
...
select t.request_at as 'Day', isnull(cast(c.canl_req/t.total_req as decimal(5,2)), 0)  as 'Cancellation Rate' from total_request t
